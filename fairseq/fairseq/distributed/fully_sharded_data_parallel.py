...
def fsdp_enable_wrap(cfg: DistributedTrainingConfig):
    ...
    if cfg.memory_efficient_bf16:
        assert cfg.bf16  # memory_efficient_fp16 should imply bf16
    group = dist_utils.get_data_parallel_group()
    if group is None and cfg.distributed_world_size == 1:
        group = DummyProcessGroup(rank=0, size=1)

    mixed_precision = (cfg.fp16 and not cfg.memory_efficient_fp16) or (cfg.bf16 and not cfg.memory_efficient_bf16)
    if cfg.fp16:
    ...
    fsdp_config = {
        ...
        "compute_device": torch.device('cuda:{}'.format(torch.cuda.current_device()))
    }
    with enable_wrap(
