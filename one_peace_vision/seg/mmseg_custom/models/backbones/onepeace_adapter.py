...
class OnePeaceAdapter(OnePeace):
    def __init__(
        self,
        pretrain_size=256,
        conv_inplane=64,
        ...
        **kwargs
        ...
        self.num_block = self.encoder.num_layers
        self.pretrain_size = (pretrain_size, pretrain_size)
        self.flags = [i for i in range(-1, self.num_block, self.num_block // 4)][1:]
        self.interaction_indexes = interaction_indexes
        self.add_vit_feature = add_vit_feature
        embed_dim = self.encoder.layers[0].embed_dim

        self.level_embed = nn.Parameter(torch.zeros(3, embed_dim))
        self.spm = SpatialPriorModule(inplanes=conv_inplane, embed_dim=embed_dim, with_cp=False)
        self.interactions = nn.Sequential(*[
          InteractionBlock(dim=embed_dim, num_heads=deform_num_heads, n_points=n_points,
                           init_values=init_values, drop_path=kwargs["drop_path_rate"],
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.interactions):
            ...
            x, c, cls = layer(x, c, cls, self.encoder.layers[indexes[0]:indexes[-1] + 1],
