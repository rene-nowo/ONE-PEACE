...
@dataclass
class AdjustCosineLRScheduleConfig(FairseqDataclass):
...
@register_lr_scheduler("adjust_cosine", dataclass=AdjustCosineLRScheduleConfig)
class AdujstCosineLRSchedule(FairseqLRScheduler):
    ...
    def __init__(self, cfg: AdjustCosineLRScheduleConfig, fairseq_optimizer):
        ...
        if self.period <= 0 and cfg.max_update > 0:
            self.period = cfg.max_update - cfg.warmup_updates
    ...
    def step_update(self, num_updates):
        ...
        if self.period <= 0 and self.cfg.max_update <= 0:
            self.lr = 0
        elif num_updates < self.warmup_updates:
    ...
    def reinit(self, total_num_update, num_updates):
        ...
        self.max_update = total_num_update
        self.warmup_updates = int(self.max_update * self.cfg.warmup_ratio)
        self.period = self.max_update - self.warmup_updates
        self.lr_step = (self.max_lr - self.cfg.warmup_init_lr) / self.warmup_updates
        if num_updates > 0:
        ...
        print('Total steps {}, warmup steps {}, warmup_factor {}'.format(self.max_update, self.warmup_updates,
                                                                         self.warmup_factor))
